...
#ifndef GOOFER_MULTI_TIMER_H
#define GOOFER_MULTI_TIMER_H
...
class GOOFER_API MultiTimer : private Uncopy
{
public:
    MultiTimer();
    ~MultiTimer();
...
size_t MultiTimer::create_timer_mem(T & t, F f, const P & p)
{
    ...
    GOOFER_NEW(timer, FuncTimer(t, f, p));
    if (nullptr == timer)
    {
        return (BAD_TIMER_ID);
    }
...
size_t MultiTimer::create_timer_mem(T & t, F f)
{
    ...
    ITimer * timer = nullptr;
    GOOFER_NEW(timer, FuncTimer(t, f));
    if (nullptr == timer)
    {
        return (BAD_TIMER_ID);
    }
...
size_t MultiTimer::create_timer_ptr(F f, P p)
{
    ...
    ITimer * timer = nullptr;
    GOOFER_NEW(timer, FuncTimer(f, p));
    if (nullptr == timer)
    {
        return (BAD_TIMER_ID);
    }
