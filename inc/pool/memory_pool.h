...
#ifndef GOOFER_MEMORY_POOL_H
#define GOOFER_MEMORY_POOL_H
...
MemoryPool<LockerType>::MemoryPool()
    ...
    , m_memory(nullptr)
{
    GOOFER_NEW_ARR(m_memory, MemoryList, m_block_kinds);
...
MemoryNode * MemoryPool<LockerType>::acquire(size_t length)
{
    ...
    char * buffer = nullptr;
    GOOFER_NEW_ARR(buffer, char, adjust_size);
    if (nullptr != buffer)
    {
