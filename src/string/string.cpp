...
size_t goofer_vsnprintf(char * buffer, size_t bufsiz, const char * fmt, va_list args)
{
...
size_t goofer_snprintf(char * buffer, size_t bufsiz, const char * fmt, ...)
{
    ...
    size_t ret_siz = goofer_vsnprintf(buffer, bufsiz, fmt, args);
...
void goofer_string_trim_tail(std::string & str, const char * trim)
{
    if (nullptr == trim || '\0' == trim[0])
    {
        return;
...
void goofer_string_trim(std::string & str, const char * trim)
{
    if (nullptr == trim || '\0' == trim[0])
    {
        return;
...
void goofer_string_simplify(std::string & str, const char * trim, char simplify)
{
    if (nullptr == trim || '\0' == trim[0])
    {
        return;
...
bool goofer_string_to_##type(const char * str, type & val)          \
{                                                                   \
    if (nullptr == str)                                             \
    {                                                               \
        return (false);                                              \
...
bool goofer_##type##_to_string(type val, std::string & str)         \
{                                                                   \
    const int bufsiz = 64;                                          \
    char buffer[bufsiz];                                            \
