...
void goofer_create_directory_recursive(const std::string & dirname)
{
    ...
    while (iter_f != iter_e)
    {
        ...
        goofer_mkdir_safe(fore_dir_name.c_str());
    }
}

void goofer_remove_directory_recursive(const std::string & dirname)
{
    if (dirname.empty())
    {
        return;
    ...
    while (!folder_list.empty())
    {
        ...
        while (dir.read())
        {
            ...
            {
                goofer_chmod_safe(dir.sub_path_name().c_str());
                goofer_unlink_safe(dir.sub_path_name().c_str());
            }
        }
        dir.close();
    }
    ...
    {
        goofer_rmdir_safe(iter->c_str());
    }
    goofer_rmdir_safe(dir_name.c_str());
}

void goofer_directory_format_to_windows(std::string & dirname)
{
    std::replace(dirname.begin(), dirname.end(), '/', '\\');
}

void goofer_directory_format_to_unix(std::string & dirname)
{
    std::replace(dirname.begin(), dirname.end(), '\\', '/');
}
...
std::string goofer_pathname_format_strictly_to_windows(const std::string & pathname)
{
...
std::string goofer_pathname_format_strictly_to_unix(const std::string & pathname)
{
    std::string pathname_strictly;
...
bool goofer_absolute_pathname_format_strictly(const std::string & src_pathname, std::string & dst_pathname, bool support_long_path)
{
    ...
    goofer_split_piece(src_pathname, "\\/", false, src_path_node_list);
    ...
    if (start_with_directory_separator)
    {
        ...
        if ("?" == first_path_node)
        {
            ...
            else if (0 == goofer_strcmp_ignore_case(second_path_node.c_str(), "UNC"))
            {
                // mode: 7, 8
                src_path_node_list.pop_front();
                if (src_path_node_list.empty())
