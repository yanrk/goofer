...
static bool exclusive_init(const char * exclusive_unique_name, size_t & unique_id, bool global_exclusive)
{
    ...
    if (ERROR_ALREADY_EXISTS == goofer_system_error() || ERROR_ACCESS_DENIED == goofer_system_error())
    {
    ...
    {
        RUN_LOG_ERR("create mutex %s failed: %d", exclusive_file.c_str(), goofer_system_error());
        return (false);
    }
    ...
    if (fd < 0)
    {
        RUN_LOG_ERR("open %s failed: %d", exclusive_file.c_str(), goofer_system_error());
        return (false);
    }
    ...
    {
        if (EACCES == goofer_system_error() || EAGAIN == goofer_system_error() || EDEADLK == goofer_system_error())
        {
            RUN_LOG_ERR("another process has started");
        }
        else
        {
            RUN_LOG_ERR("lock %s failed: %d", exclusive_file.c_str(), goofer_system_error());
        }
        ::close(fd);
        return (false);
    }
    ...
    if (::ftruncate(fd, 0) < 0)
    {
        RUN_LOG_ERR("truncate %s failed: %d", exclusive_file.c_str(), goofer_system_error());
    }

    if (::write(fd, pid.c_str(), pid.size()) < 0)
    {
        RUN_LOG_ERR("write %s failed: %d", exclusive_file.c_str(), goofer_system_error());
    }
...
static bool exclusive_is_exist(const char * exclusive_unique_name, bool global_exclusive)
{
    ...
    bool is_exist = (ERROR_ALREADY_EXISTS == goofer_system_error() || ERROR_ACCESS_DENIED == goofer_system_error());
    if (nullptr != mutex)
    {
        ::CloseHandle(mutex);
    }
    ...
    if (::fcntl(fd, F_SETLK, &locker) < 0)
    {
        is_exist = (EACCES == goofer_system_error() || EAGAIN == goofer_system_error() || EDEADLK == goofer_system_error());
    }
