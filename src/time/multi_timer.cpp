...
{
    uint64_t diff = 0;
    struct timeval etime = goofer_gettimeofday();
    diff += (etime.tv_sec - stime.tv_sec) * 1000;
    diff += (etime.tv_usec - stime.tv_usec) / 1000;
    return (diff);
}

static thread_return_t GOOFER_STDCALL multi_timer_run(thread_argument_t argument)
{
...
void MultiTimer::destroy_timer(size_t id)
{
    ...
    GOOFER_DEL(running_timer);
    ITimer * pending_timer = del_pending_timer(id);
    GOOFER_DEL(pending_timer);
}
...
{
    struct timeval stime = goofer_gettimeofday();
